...
Status: reviewed; as built with tracked gaps
...
The interface sends the prior conversation history (up to six messages are included in the provider prompt). `ChatRequest` accepts at most 12 history entries, but the browser currently sends its entire in-memory history without truncation. After seven completed exchanges, the next request can exceed the schema limit and receive HTTP 422; this implementation gap is tracked by TASK-008. The provider key is read from `GROQ_API_KEY`; the model is configured with `GROQ_MODEL` and defaults to `openai/gpt-oss-20b`.

## Retrieval and Generation

Retrieval uses a local `SentenceTransformer` model (`sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2` by default) to embed the policy texts and each Spanish question. It ranks normalized vectors by cosine similarity, returns up to three results above `EMBEDDING_MIN_SIMILARITY` (default `0.4`), and abstains without calling Groq when none pass the threshold. The model and corpus embeddings are cached in process; changing the corpus invalidates its embedding cache. `EMBEDDING_MODEL` and `EMBEDDING_MIN_SIMILARITY` can be configured through the environment. This follows the reference project's Sentence Transformers embedding approach while using in-process cosine search instead of adding a FAISS dependency for the small policy corpus.
...
The view has starter suggestions, a message list, a searching state, retrieved sources, readable errors, Enter-to-send, and a new-conversation button. The CSS uses grid, gradient backgrounds, mint accents, and a 720 px media query. On mobile, it hides the sidebar and shows a reset control in the header. Bootstrap and fonts load from CDNs, so the view requires Internet access for those assets.
...
- Pending: evaluate representative semantic paraphrases with the configured model, inspect desktop/mobile browser rendering, and exercise `/api/chat` with a valid key.
...
- The initial knowledge base has only three demonstration policies; questions outside them are rejected.
- The browser/server history limit mismatch can make longer conversations fail with HTTP 422; resolve via TASK-008.
