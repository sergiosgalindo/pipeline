...
- The configured model was evaluated on Spanish paraphrases for loans, late returns, and reserve materials, plus an out-of-corpus question that correctly abstained. A live server check of `/`, `/api/health`, and `/api/chat` succeeded with the configured Groq key. Desktop (1280×800) and mobile (390×844) rendering were inspected: no horizontal overflow, the composer stays in view, the conversation scrolls, Enter sends a message, and a prompt card tap sends a message. The mobile header shows the reset control and hides the sidebar.
...
- Questions outside the 27 configured policies are rejected. A vague question can retrieve up to three related policies, such as different loan durations, and the answer must follow the policy that names the material.
