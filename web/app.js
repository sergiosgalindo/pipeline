const MAX_SUBMITTED_HISTORY = 12;

function submittedHistory(items) {
  return items.slice(-MAX_SUBMITTED_HISTORY);
}
  ...
  async function ask(question) {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ message: question, history: submittedHistory(priorHistory) }),
      });
